...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'POST') {
      ...
      if (!superheroId) return res.status(400).json({ message: 'Superhero ID is required' });
      ...
      if (isNaN(superheroIdNum)) return res.status(400).json({ message: 'Invalid Superhero ID' });
